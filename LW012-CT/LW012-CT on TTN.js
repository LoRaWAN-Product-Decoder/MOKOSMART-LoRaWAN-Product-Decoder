var payloadTypeArray = [
    ...
    , "GPS Limit"
    , "Anti-demolition"];
var operationModeArray = ["Standby mode", "Periodic mode", "Timing mode", "Motion mode", "Timing + period mode"];
...
function decodeUplink(input) {
    ...
    if (fPort == 1 && bytes.length == 9) {
        ...
        parse_port2_data(data, bytes.slice(3),contain_vlotage);
        ...
        data.payload_type = payloadTypeArray[8];
...
function parse_port4_data(data, bytes, contain_vlotage) {
    ...
    if (failedTypeCode == 0 || failedTypeCode == 1 || failedTypeCode == 2
        ...
        for (var i = 0; i < number; i++) {
            ...
            item.rssi = rssi;
            if (contain_vlotage) {
                item.voltage = bytesToInt(sub_bytes, 7, 2) + "mV";
            }
            data_list.push(item);
        }
...
function parse_position_data(bytes, contain_vlotage) {
    ...
    for (var i = 0; i < number; i++) {
        ...
        obj.rssi = rssi;
        if (contain_vlotage) {
            obj.voltage = bytesToInt(sub_bytes, 7, 2) + "mV";
        }
        mac_data.push(obj);
    }
    return mac_data;
