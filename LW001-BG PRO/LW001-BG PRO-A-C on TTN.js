...
function decodeUplink(input) {
	...
	if (port <= 10) {
		...
		var humidity = bytesToInt(bytes, 2, 1) + '%';
		data.temperature = temperature;
		data.humidity = humidity;

		data.ack = bytes[3] & 0x0f;
		data.battery_voltage = (22 + ((bytes[3] >> 4) & 0x0f)) / 10 + "V";
	}
	if (port == 1) {
		var rebootReasonCode = bytesToInt(bytes, 4, 1);
		// data.reboot_reason_code = rebootReasonCode;
		data.reboot_reason = rebootReasonArray[rebootReasonCode];

		var majorVersion = (bytes[5] >> 6) & 0x03;
		var minorVersion = (bytes[5] >> 4) & 0x03;
		var patchVersion = bytes[5] & 0x0f;
		var firmwareVersion = 'V' + majorVersion + '.' + minorVersion + '.' + patchVersion;
		data.firmware_version = firmwareVersion;

		var activityCount = bytesToInt(bytes, 6, 4);
		...
		var eventTypeCode = bytesToInt(bytes, 4, 1);
