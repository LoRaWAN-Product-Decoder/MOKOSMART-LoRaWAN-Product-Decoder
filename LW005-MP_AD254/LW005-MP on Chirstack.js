...
function Decode(fPort, bytes) {
    ...
    switch (fPort) {
        case 5:
            ...
            dev_info.instantaneous_voltage = bytesToInt(bytes, 7, 2) / 10 + "V";
            dev_info.instantaneous_current = bytesToInt(bytes, 9, 2) / 1000 + "A";
            dev_info.instantaneous_current_frequency = bytesToInt(bytes, 11, 2) / 1000 + "HZ";
            dev_info.instantaneous_active_power = bytesToInt(bytes, 13, 4) / 10 + "W";
            dev_info.instantaneous_power_factor = (bytes[17] & 0xFF) + "%";
            dev_info.total_energy = bytesToInt(bytes, 18, 4) / 3200 + "KWH";
            dev_info.energy_of_last_hour = bytesToInt(bytes, 22, 2) / 3200 + "KWH";
        case 9:
