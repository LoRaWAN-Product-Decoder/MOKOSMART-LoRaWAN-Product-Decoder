...
function decodeUplink(input) {
    ...
    switch (fPort) {
        case 5:
            ...
            data.instantaneous_voltage = bytesToInt(bytes, 7, 2) / 10 + "V";
            data.instantaneous_current = bytesToInt(bytes, 9, 2) / 1000 + "A";
            data.instantaneous_current_frequency = bytesToInt(bytes, 11, 2) / 1000 + "HZ";
            data.instantaneous_active_power = bytesToInt(bytes, 13, 4) / 10 + "W";
            data.instantaneous_power_factor = (bytes[7] & 0xFF) + "%";
            data.total_energy = Number(bytesToInt(bytes, 18, 4) / 3200).toFixed(2) + "KWH";
            data.energy_of_last_hour = Number(bytesToInt(bytes, 22, 2) / 3200).toFixed(2) + "KWH";
            break;
        case 9:
