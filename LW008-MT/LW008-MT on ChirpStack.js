function Decode(fPort, bytes) {
    ...
    if (fPort == 12) {
        return parse_port12_data(bytes, fPort);
    }
    var deviceInfo = {};
    deviceInfo['fPort'] = fPort;
    ...
    if (fPort == 1 && (bytes.length == 9)) {
        data_dic = parse_port1_data(bytes.slice(3), fPort);
    } else if (fPort == 2 && (bytes.length >= 7)) {
        data_dic = parse_port2_data(bytes.slice(3), fPort);
    } else if (fPort == 4 && (bytes.length >= 5)) {
        data_dic = parse_port4_data(bytes.slice(3), fPort);
        ...
        data_dic = parse_port9_data(bytes.slice(3), fPort);
    }
...
function parse_port2_data(bytes, fPort) {
    if (bytes.length < 4 || fPort != 2) {
        return {};
    }
...
function parse_port4_data(bytes, fPort) {
    if (bytes.length < 2 || fPort != 4) {
        return {};
    }
...
function parse_port9_data(bytes, fPort) {
    if (bytes.length != 40 || fPort != 9) {
        return {};
    }
...
function parse_port12_data(bytes, fPort) {
    if (fPort != 12 || (bytes.length != 11)) {
        return {};
    }
    var data_dic = { 'fPort': fPort };
