...
function Decoder(bytes, fPort, groupID) {
    ...
    if (fPort == 3 && bytes.length == 8) {
        ...
        return __spreadArray(__spreadArray([], payloadList, true), parse_port3_data(bytes.slice(3), groupID), true);
    }
...
function parse_port2_data(bytes, groupID) {
    ...
    if (positionTypeCode == 2) {
        var positionData = parse_position_data(sub_bytes);
...
function parse_position_data(bytes) {
...
var payload_raw = payload.find(function (x) { return x.variable === "payload"; });
var port = payload.find(function (x) { return x.variable === "fport"; });
if (payload_raw && port) {
    try {
        // Convert the data from Hex to Javascript Buffer.
        var buffer = hexToNumberArray(payload_raw.value);
