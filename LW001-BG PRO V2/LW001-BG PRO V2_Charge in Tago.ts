...
function Decoder(bytes: number[], fPort: number, groupID: string): { [key: string]: any }[] {
	...
	if (fPort <= 10) {
		...
		const charging_status_str = charging_status == 0 ? "No charging" : "Charging"
		payloadList.push(getPayloadData('charging_status', charging_status_str, groupID));
		...
		const battery_voltage = ((28 + ((bytes[3] >> 4) & 0x0f)) / 10).toString() + 'V';
		payloadList.push(getPayloadData('battery_voltage', battery_voltage, groupID));
	
		var battery_percent = (bytes[4] & 0xff).toString() + "%";
	...
	if (fPort == 1) {
		...
		const battery_voltage = ((28 + ((bytes[1] >> 4) & 0x0f)) / 10).toFixed(1).toString() + 'V';
		payloadList.push(getPayloadData('battery_voltage', battery_voltage, groupID));
