...
function Decoder(bytes: number[], fPort: number, groupID: string):{ [key: string]: any }[] {
    ...
    const payload_type = payloadTypeArray[fPort - 5];
...
function parse_int16(num:number) {
    if (num & 0x8000)
        return (num - 0x10000);
    else
        return num;
...
function parse_int24(num:number) {
    if (num & 0x800000)
